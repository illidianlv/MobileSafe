...
public class TaskManagerActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		lv_task_manager.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				TaskInfo taskInfo;
				if (position == 0) {// 用户进程小标签
					return;
				} else if (position == userTaskInfos.size() + 1) {// 系统进程小标签
					return;
				...
				if (taskInfo.isChecked()) {
					...
					holder.cb_stauts.setChecked(false);
				} else {
					taskInfo.setChecked(true);
					holder.cb_stauts.setChecked(true);
				}
	...
	private class TaskManagerAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView != null && convertView instanceof RelativeLayout) {
				...
				holder.cb_stauts = (CheckBox) view.findViewById(R.id.cb_status);
				view.setTag(holder);
			}
			...
			holder.tv_memsize.setText("内存占用"
					+ Formatter.formatFileSize(getApplicationContext(),
							taskInfo.getMemsize()));
			holder.cb_stauts.setChecked(taskInfo.isChecked());
			if (getPackageName().equals(taskInfo.getPackname())) {
				holder.cb_stauts.setVisibility(View.INVISIBLE);
			} else {
				holder.cb_stauts.setVisibility(View.VISIBLE);
			}
			return view;
		}
	}
	...
	public void killAll(View view) {
		...
		Toast.makeText(
				this,
				"杀死了" + count + "个进程." + "释放了"
						+ Formatter.formatFileSize(this, saveMem) + "内存", 1)
				.show();
		processCount -= count;
		availMem += saveMem;
		tv_process_count.setText("运行中的进程:" + processCount + "个");
